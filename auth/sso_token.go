...
// ImportFromSsoToken Import tài khoản từ SSO Token (x-amz-sso_authn)
func ImportFromSsoToken(bearerToken, region string) (accessToken, refreshToken, clientID, clientSecret string, expiresIn int, err error) {
	...
	// 1. Đăng ký OIDC client
	clientID, clientSecret, err = registerDeviceClient(oidcBase, startUrl)
	if err != nil {
		return "", "", "", "", 0, fmt.Errorf("client registration failed: %w", err)
	}

	// 2. Khởi tạo device authorization
...
func GetUserInfo(accessToken string) (email, userID string, err error) {
	// Gọi Kiro API lấy thông tin usage (bao gồm user info)
...
// GenerateAccountID Tạo ID tài khoản
