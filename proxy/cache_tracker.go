...
type promptCacheRouteCounters struct {
	Requests                 int
	Hits                     int
	Misses                   int
	CacheReadInputTokens     int
...
type promptCacheRouteStats struct {
	...
	ExpiresSoonest           int64  `json:"expiresSoonest,omitempty"`
}
...
type promptCacheStats struct {
	...
	CacheReadInputTokens     int                     `json:"cacheReadInputTokens"`
	CacheCreationInputTokens int                     `json:"cacheCreationInputTokens"`
	Routes                   []promptCacheRouteStats `json:"routes"`
}

type promptCacheTracker struct {
	mu               sync.Mutex
	entriesByAccount map[string]map[[32]byte]promptCacheEntry
	statsByAccount   map[string]promptCacheRouteCounters
...
func newPromptCacheTracker(maxTTL time.Duration) *promptCacheTracker {
	...
	return &promptCacheTracker{
		...
		statsByAccount:   make(map[string]promptCacheRouteCounters),
...
func (t *promptCacheTracker) Compute(accountID string, profile *promptCacheProfile) promptCacheUsage {
	...
	if len(entries) == 0 {
		...
		t.recordUsageLocked(accountID, usage)
		return usage
	}
...
func (t *promptCacheTracker) recordUsageLocked(accountID string, usage promptCacheUsage) {
	counters := t.statsByAccount[accountID]
	counters.Requests++
	...
	counters.CacheReadInputTokens += usage.CacheReadInputTokens
	counters.CacheCreationInputTokens += usage.CacheCreationInputTokens
	t.statsByAccount[accountID] = counters
}
...
func (t *promptCacheTracker) Stats() promptCacheStats {
	...
	for routeID := range t.entriesByAccount {
		routeIDs[routeID] = true
	}
	for routeID := range t.statsByAccount {
		routeIDs[routeID] = true
	}
	...
	result := promptCacheStats{
		Routes: make([]promptCacheRouteStats, 0, len(ids)),
	}
	for _, routeID := range ids {
		entries := t.entriesByAccount[routeID]
		counters := t.statsByAccount[routeID]
		route := promptCacheRouteStats{
			RouteID:                  routeID,
			Entries:                  len(entries),
		...
		for _, entry := range entries {
			...
			if route.ExpiresSoonest == 0 || expiresAt < route.ExpiresSoonest {
				route.ExpiresSoonest = expiresAt
			}
		}
		result.EntryCount += route.Entries
		result.Requests += route.Requests
		result.Hits += route.Hits
		result.Misses += route.Misses
		result.CacheReadInputTokens += route.CacheReadInputTokens
		result.CacheCreationInputTokens += route.CacheCreationInputTokens
		result.Routes = append(result.Routes, route)
	}
	result.RouteCount = len(result.Routes)
	return result
}
