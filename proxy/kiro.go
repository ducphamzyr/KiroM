...
func getSortedEndpointsForAccount(preferred string, account *config.Account) []kiroEndpoint {
	endpoints := getSortedEndpoints(preferred)
	region := profileArnRegion(account)
	if region == "" || region == "us-east-1" {
		return endpoints
	}
	result := make([]kiroEndpoint, len(endpoints))
	for i, ep := range endpoints {
		ep.URL = strings.Replace(ep.URL, "us-east-1", region, 1)
		result[i] = ep
	}
	return result
}

func profileArnRegion(account *config.Account) string {
	if account == nil {
		return ""
	}
...
func CallKiroAPI(account *config.Account, payload *KiroPayload, callback *KiroStreamCallback) error {
	...
	endpoints := getSortedEndpointsForAccount(config.GetPreferredEndpoint(), account)
