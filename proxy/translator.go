...
// Map model (có thứ tự, key dài match trước, tránh "claude-sonnet-4" match nhầm "claude-sonnet-4.5")
...
// ParseModelAndThinking Parse tên model, trả về model thực tế và có bật thinking không
func ParseModelAndThinking(model string, thinkingSuffix string) (string, bool) {
	lower := strings.ToLower(model)
	thinking := false

	// Kiểm tra bằng hậu tố đã cấu hình
	...
	// Map model (match có thứ tự, key dài ưu tiên)
	...
	// Nếu đã là model Kiro hợp lệ, trả về trực tiếp
...
func OpenAIToKiro(req *OpenAIRequest, thinking bool) *KiroPayload {
	...
	// Nếu bật thinking mode, inject thinking prompt
	if thinking {
	...
	for i, msg := range nonSystemMessages {
		...
		case "user":
			...
			// Tin nhắn user đầu tiên merge system prompt
		...
		case "tool":
			...
			// Kiểm tra tin tiếp theo có phải tool không
			nextIdx := i + 1
...
// extractThinkingFromContent Trích xuất nội dung trong tag <thinking> từ content
func extractThinkingFromContent(content string) (string, string) {
	...
	for {
		...
		// Trích xuất nội dung thinking
		thinkingContent := result[start+10 : end]
		reasoning += thinkingContent

		// Xóa tag thinking khỏi kết quả
...
// KiroToOpenAIResponseWithReasoning Phản hồi OpenAI có reasoning_content
func KiroToOpenAIResponseWithReasoning(content, reasoningContent string, toolUses []KiroToolUse, inputTokens, outputTokens int, model, thinkingFormat string) map[string]interface{} {
	...
	if len(toolUses) > 0 {
		...
		// Định dạng output thinking theo cấu hình
		if reasoningContent != "" {
